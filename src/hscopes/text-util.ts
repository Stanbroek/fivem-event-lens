...
export function toRangeDelta(oldRange: vscode.Range, text: string): RangeDelta {
    ...
    if (oldRange.start.line === oldRange.end.line) {
        charsDelta =
            newEnd.character -
            (oldRange.end.character - oldRange.start.character);
    } else {
...
export function rangeDeltaNewRange(delta: RangeDelta): vscode.Range {
    ...
    return new vscode.Range(
        delta.start,
        new vscode.Position(delta.end.line + delta.linesDelta, x)
    );
}

function positionRangeDeltaTranslate(
    pos: vscode.Position,
    delta: RangeDelta
): vscode.Position {
...
function positionRangeDeltaTranslateEnd(
    pos: vscode.Position,
    delta: RangeDelta
): vscode.Position {
